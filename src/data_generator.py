...
class Imagenet64(object):
    def __init__(self, data_path):
        ...
        for train_file in train_files:
            ...
            with open(self.data_path / "train_data" / train_file, "rb") as fo:
                ...
                x = (
                    ...
                    .transpose((0, 2, 3, 1)) # TF expects channels last (N, H, W, C)
                )
        ...
        print(f"Finished loading training dataset with {self.train_size} samples")
        ...
        print(f"Finished loading test dataset with {self.test_size} samples")
        ...
        while True:
            ...
            for i in range(0, ds_size, batch_size):
                ...
                if not isinstance(x, np.ndarray):
                    # when augmentation is off, x is tf tensor
                    x = x.numpy()
                x = x.transpose((0, 3, 1, 2)) # pytorch works better with (N, C, H, W)
                yield x, y

            self.epoch_i += 1

class Imagenet64Eval(object):
    def __init__(self, eval_data_file_path):
        self.eval_data_file_path = Path(str(eval_data_file_path))

        with open(self.eval_data_file_path, "rb") as fo:
            data = pickle.load(fo)
            x_test = (
                data["data"]
                .reshape((data["data"].shape[0], 3, 64, 64))
    ...
    def datagen_cls(self, batch_size):

        for i in range(0, self.test_size, batch_size):
            x, y = self.data["x_test"][i:i + batch_size], self.data["y_test"][i:i + batch_size]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Dataset generator test.")
    ...
    #dg = ds.datagen_cls(10, augmentation=True)
    dg = ds.datagen_cls(10, ds='test', augmentation=False)
